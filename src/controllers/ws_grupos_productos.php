...
include_once ('src/funciones/igtech.funciones_grupos_productos.php');

$app->get('/grupos_ventas/{empresa}',function(Request $request,Response $response){
    ...
    try{
        ...
        if(/*$w_datos_token->respuesta == 0 and */ $w_autorizacion['datos']['estado']=='V'){
            $o_respuesta=listaGrupoProductosVentas($i_empresa);
        }else{
...
$app->get('/grupos_compras/{empresa}',function(Request $request,Response $response){
    $i_token=$request->getHeaderLine('Authorization');
    $i_ct=$request->getHeaderLine('Content-Type');
    $i_accept=$request->getHeaderLine('Accept');
    $i_empresa =$request->getAttribute('empresa');
    try{
        ...
        $w_autorizacion= seleccionarToken($i_token);
        if(/*$w_datos_token->respuesta == 0 and */ $w_autorizacion['datos']['estado']=='V'){
            $o_respuesta=listaGrupoProductosCompras($i_empresa);
        }else{
            $o_respuesta=array('error'=>'9998','mensaje'=>'token invalido');
        }
    }catch (Throwable $e) {
