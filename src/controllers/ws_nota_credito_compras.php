...
include_once ('src/funciones/igtech.funciones_nccompras.php');

$app->get('/notasCreditoCompras/{empresa}/{anio}/{mes}',function(Request $request,Response $response){
  ...
  try{
    ...
    if(/*$w_datos_token->respuesta == 0 and */ $w_autorizacion['datos']['estado']=='V'){
      $o_respuesta=listaNotasCreditoCompras($i_empresa,$i_anio,$i_mes);
    }else{
...
$app->get('/detalleNCCompras/{empresa}/{documento}',function(Request $request,Response $response){
  $i_token=$request->getHeaderLine('Authorization');
  $i_ct=$request->getHeaderLine('Content-Type');
  $i_accept=$request->getHeaderLine('Accept');
  $i_empresa =$request->getAttribute('empresa');
  ...
  try{
    ...
    $w_autorizacion= seleccionarToken($i_token);
    if(/*$w_datos_token->respuesta == 0 and */ $w_autorizacion['datos']['estado']=='V'){
      $o_respuesta=listaDetalleNCCompras($i_empresa,$i_documento);
    }else{
      $o_respuesta=array('error'=>'9998','mensaje'=>'token invalido');
    }
  }catch (Throwable $e) {
...
$app->get('/detalleNCComprasDebi/{empresa}/{documento}',function(Request $request,Response $response){
  $i_token=$request->getHeaderLine('Authorization');
  $i_ct=$request->getHeaderLine('Content-Type');
  $i_accept=$request->getHeaderLine('Accept');
  $i_empresa =$request->getAttribute('empresa');
  ...
  try{
    ...
    $w_autorizacion= seleccionarToken($i_token);
    if(/*$w_datos_token->respuesta == 0 and */ $w_autorizacion['datos']['estado']=='V'){
      $o_respuesta=listaDetalleNCComprasDebi($i_empresa,$i_documento);
    }else{
      $o_respuesta=array('error'=>'9998','mensaje'=>'token invalido');
    }
  }catch (Throwable $e) {
